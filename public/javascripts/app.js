...
App = (function() {
  ...
  App.prototype.bindEvents = function() {
    ...
    return $(".category").on("click", function(e) {
      var items;
  ...
  App.prototype.tagify = function(tag, content) {
    ...
    if (matches && matches[0].length === tag.length && !matches[0].match(/title/)) {
      ...
      node = "div";
      if ((attributes && attributes.match(/href/)) || content.indexOf('http') === 0 || content.indexOf('mailto:') === 0) {
        node = "a";
  ...
  App.prototype.htmlify = function(object, prettify) {
    ...
    if (prettify == null) prettify = false;
    result = "";
    ...
    if (prettify) result = indent(result, null);
    return result;
  };
